...
MIN_SAME_DIRECTION_STEPS = 4
MAX_SAME_DIRECTION_STEPS = 10
...
def dijkstra(matrix):
    ...
    while len(minHeap) > 0:
        ...
        if i == len(matrix) - 1 and j == len(matrix[0]) - 1 and streak >= MIN_SAME_DIRECTION_STEPS:
            return heat
...
def getPossibleDirections(di, dj, streak):
    ...
    for direction in DIRECTIONS:
        if direction != (di, dj) and direction != (-di, -dj) and streak >= MIN_SAME_DIRECTION_STEPS:
